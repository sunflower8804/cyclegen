...
class StarClanScreen(Screens):
    ...
    def get_dead_cats(self):
        ...
        for the_cat in Cat.all_cats_list:
            if the_cat.dead and the_cat.ID != game.clan.instructor.ID and the_cat.ID != game.clan.demon.ID and not the_cat.outside and not the_cat.df and not the_cat.faded:
    ...
    def update_page(self):
        ...
        if self.current_listed_cats:
            for cat in self.chunks(self.current_listed_cats, 20)[self.list_page - 1]:
                ...
                self.cat_names.append(pygame_gui.elements.ui_label.UILabel(ui_scale(pygame.Rect((160 + pos_x, 460 + pos_y), (300, 60))), short_name, object_id="#text_box_30_horizcenter_light", manager=MANAGER))
                pos_x += 240
                if pos_x >= 1200:
                    pos_x = 0
                    pos_y += 200
